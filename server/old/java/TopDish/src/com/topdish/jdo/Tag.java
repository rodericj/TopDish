 ...
 */
@PersistenceCapable
public class Tag implements TDPersistable, Serializable {
	...
	@Persistent
	private Integer type = TYPE_GENERAL; // set default value since null causes
	 ...
	 */
	public Tag(Key parentTag, String name, String description, int type, Date dateCreated,
			Key creator) throws CuisineCannotHaveParentException {
	 ...
	 * 
	 * Note: do not use for creating a Tag object. Specifically created for
	 * generic operations.
	 */
	public Tag() {
	}
	...
	public JSONObject toJSONLite() {
		...
		try {
			...
			jsonObj.put(TagUtils.KEY, key.toString()).put(TagUtils.NAME, name)
					.put(TagUtils.DESCRIPTION, description).put(TagUtils.MANUAL_ORDER, manualOrder);
