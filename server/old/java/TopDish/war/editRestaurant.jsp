...
<%	
	...
	Restaurant found = Datastore.get(KeyFactory.createKey(Restaurant.class.getSimpleName(), restID));
%>
<h2>Edit Restaurant</h2>
<div class="restaurant_photo_large">
<%		if(!found.getPhotos().isEmpty()){
			Photo p = Datastore.get(found.getPhotos().get(0));
...
<form action="updateRestaurant" method="post">
	...
	<select name="cuisine_id" id="cuisine">
		<%
			...
			if(null == found.getCuisine()){
				System.out.println("CUISINE IS NULL");
				out.print("<option value=\"\" selected>Please Select One</option>\n");
			}
			
			for (Tag c : cuisines) {
				if(null != found.getCuisine() && c.getKey().getId() == found.getCuisine().getId()){
					out.print("<option value=\"" + c.getKey().getId() + "\" selected >" + c.getName() + "</option>\n");
				}else{
    ...
    <%
System.out.println("MADE IT GREEN");
%>
