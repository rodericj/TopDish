...
<%
	...
	if (dishIDs != null) {
		...
		final Dish d = Datastore.get(KeyFactory.createKey(
				Dish.class.getSimpleName(), dishID));
		int posReviews = 0;
		int negReviews = 0;
		final Set<Key> tagKeys = d.getTags();
		final TDUser creator = Datastore.get(d.getCreator());
		final Review firstReview = TDQueryUtils.getFirstReviewByDish(d
				.getKey());
		final Review lastReview = TDQueryUtils.getLatestReviewByDish(d
				.getKey());
		TDUser firstReviewAuthor = null;

		if (firstReview != null)
			firstReviewAuthor = Datastore.get(firstReview.getCreator());
...
<div class="rmenu_cont dish_splitter">
	...
	<div class="vote_stats">
		<div class="like_stats">
			<h2>
				<img src="img/up_arrow_large.png" width="16" height="16"
					alt="Like it" />
				<%=posReviews%>
				<%
					out.print((posReviews != 1) ? "Like" : "Likes");
				%>
				it
			</h2>
			<h3><%=posReviews%>
				<%
					out.print((posReviews != 1) ? "People" : "Person");
				%>
				Rated on:
			</h3>
			<p>
		...
		<div class="dislike_stats">
			<h2>
				<img src="img/down_arrow_large.png" width="16" height="16"
					alt="Dislike it" /><%=negReviews%>
				<%
					out.print((posReviews != 1) ? "Dislike" : "Dislikes");
				%>
				it
			</h2>
			<h3><%=negReviews%>
		...
		<div class="user_credits">
			<div class="user_brief">
				...
				<div class="user_photo">
					...
					%>
					<img class="grey_icon"
						src="/getPhoto?id=<%=creator.getPhoto().getId()%>" />
					<%
						} else {
					%>
 ...
 %> </a>
			...
			<div class="user_brief">
				...
				<div class="user_photo">
					...
					%>
					<img class="grey_icon"
						src="/getPhoto?id=<%=firstReviewAuthor.getPhoto().getId()%>" />
					<%
						} else {
					%>
					<img class="grey_icon" src="style/no_user_img.jpg" />
