...
<%@ page import="com.topdish.comparator.TDUserNumReviewsComparator" %>
<%@ page import="com.topdish.util.HumanTime" %>
<%@ page import="com.topdish.util.Datastore" %>

<%
	List<TDUser> users = TDQueryUtils.getTopUsers(4);
	Collections.sort(users, new TDUserNumReviewsComparator());
	int oddEven = 0;
%>
   ...
   <div class="top_foodie_cont">
       	...
       	<div class="top_foodie_box_border<%if(oddEven % 2 == 0){%><%=" top_foodie_box_even"%><%}%>">
       		<div class="top_foodie_box">
       			<div class="photo">
	       			<% if(u.getPhoto() != null){ 
	       				Photo userPhoto = Datastore.get(u.getPhoto());
	       				final String url = userPhoto.getURL(36);%>
	       				<img src="<%=url%>"></img>
	       			<%}else{ %>
       			...
       			<div class="last_review">
					<%
					Review r = TDQueryUtils.getLatestReviewByUser(u.getKey());
					if(null != r && null != r.getDish()){
						try{
							Dish d = Datastore.get(r.getDish());
							...
							//could not find dish for that review
							e.printStackTrace();
						}
					}
					%>
