...
<%	
	...
	try{
		creator = TDUserService.getUser(session);
	...
	if(reviewID != 0 || photoID != 0){
		...
		if(reviewID != 0){
			review = Datastore.get(KeyFactory.createKey(Review.class.getSimpleName(), reviewID));
			%>
		...
		if(photoID != 0){
			photo = Datastore.get(KeyFactory.createKey(Photo.class.getSimpleName(), photoID));
			final String url = photo.getURL(200);%>
			<img src="<%=url%>">
<%		}
