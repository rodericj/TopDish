...
public class DishLite{
	...
	public DishLite(Dish d) {
		...
		if(d.getPhotos() != null && d.getPhotos().size() > 0){
			Photo dishPhoto = PMF.get().getPersistenceManager().getObjectById(Photo.class, d.getPhotos().get(0));
			this.photoURL = ImagesServiceFactory.getImagesService().getServingUrl(dishPhoto.getBlobKey(), 300, true);
		}
