...
//check if user agent is samsung or ipad
function dropDownFilter()
{
	...
	var isSamsung = userAgent.match(/SCH-I800/i) != null;
	var isGT = userAgent.match(/GT-P1000/i) != null;
	var isSPH = userAgent.match(/SPH-P100/i) != null;
	var isSC = userAgent.match(/SC-01C/i) != null;
	var isSHW = userAgent.match(/SHW-M180L/i) != null;
	if(isiPad || isSamsung || isGT || isSPH || isSC || isSHW)
		return true;
	else
		return false;
}
$(document).ready(function(){
	var ajaxCallType="";
	filterCount=0;
	var isDropDownFilter = dropDownFilter();
	ajaxCallType=$("#callTypeId").val();
	if(isDropDownFilter)
		{
			//show filter with down arrow
			$("#searchFilterId").removeClass("search_filters");
			$("#searchFilterId").addClass("search_filters_dropdown");

		}
	else
		{
			//show filter with side arrow
			$("#searchFilterId").addClass("search_filters");
			$("#searchFilterId").removeClass("search_filters_dropdown");
		}
	$('#searchFilterId').toggle(
		function() 
		{
			var $this = $(this);
			if(isDropDownFilter)
			{
				// to slide the filters down instead of on left side
				
				$('#filter_list').css("z-index","200");
				$('#filter_list').css("background-color","#f2faff");
				$('#filter_list').css("border","1px solid #5fe3ff");
				
				$('#filter_list').css("left","0");
				$('#filter_list').css("top","16px");
				$('#filter_list').css("-moz-border-radius","0px 0px 10px 10px");
				
				$('#filter_list').stop(true, true).animate({ 'bottom' : 'auto'}, "slow");
				$('#filter_list').css("height","auto");
				
				//blue border 
				$('#filter_list ul li ').css("border-top","1px solid #5fe3ff");
				$('#filter_list ul li > ul li').css("border-top","none");
				$('#filter_list ul li:first-child').css("border-top","medium none");
				$('.search_filters_dropdown').removeClass('search_filter_offset');
				
			}
			else
			{
				// to slide the filters on left side
				$('#filter_list').css("height","auto");
		...
		function()
		{
			if(isDropDownFilter)
			{
				// to slide the filters up
				$('#filter_list').css("z-index","1");
				$('#filter_list').css("background-color","#FFF");
				$('#filter_list').css("border","1px solid #CCC");
				$('#filter_list').css("left","0");
				$('#filter_list').css("top","0");
				$('#filter_list').stop(true, true).animate({ 'height' : ''}, "slow");
				$('.search_filters_dropdown').addClass('search_filter_offset');
			}
			else
			{
				// to slide the filters on the right
	...
	function closeFilter()
	{
	 ...
	 function loadTopDishesOnFilter() { 
		...
		var hiddenSearchWord="";
			hiddenSearchWord=$("#hiddenSearchWordId").val();
		
		if(hiddenSearchWord!=searchWord)
		{
			searchWord=hiddenSearchWord;
			$('#searchId').val(searchWord);
		}
		...
		if(category.length==0 && distanceFilter.length==0 && lifestyleFilter.length==0 && priceFilter.length==0)
			{
				filterPresent=false;
			}
		...
		$.ajax({
			...
			data: "callType="+ajaxCallType+"&searchWord="+searchWord+"&location="+location+"&categoryID="+categoryId+"&priceID="+priceFilterId+"&lifestyleID="+lifestyleFilterId+"&distance="+distanceFilterId+"&page="+pageNum+"&maxResults=10",
			dataType:"xml",
			cache: false,
