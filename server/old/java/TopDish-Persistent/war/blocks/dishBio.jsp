...
%>
		...
		<div class="rmenu_cont dish_splitter">
			...
			<c:if test="${param.limitedView ne 'true'}">
			<span><a href="flag.jsp?dishID=<%=d.getKey().getId()%>"><img src="img/red_flag.gif" width="8" height="12" />Flag</a> (<%=d.getNumFlagsTotal()%>)</span>
			<user:isUserInRole roles="${administrator},${advanced},${standard}" dishId="${param.dishID}">
           ...
           <div class="rmenu_disp">
           		<c:if test="${param.limitedView ne 'true'}">
           		<fb:like show_faces="false" width="4"></fb:like>
           		</c:if>
