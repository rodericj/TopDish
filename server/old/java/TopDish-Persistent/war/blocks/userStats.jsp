...
<%
	...
	if(userIDs != null){
		...
		try{
			...
			Query qPosReviews = pm.newQuery("select key from " + Review.class.getName());
			qPosReviews.setFilter("direction == " + Review.POSITIVE_DIRECTION);
			List<Key> posRevKeys = (List<Key>)qPosReviews.execute();
			long posReviews = posRevKeys.size();
			
			Query qNegReviews = pm.newQuery("select key from " + Review.class.getName());
			qNegReviews.setFilter("direction == " + Review.NEGATIVE_DIRECTION);
			List<Key> negRevKeys = (List<Key>)qNegReviews.execute();
			long negReviews = negRevKeys.size();
			
			Query qLastReview = pm.newQuery(Review.class);
			qLastReview.setFilter("creator == :creatorParam");
			qLastReview.setRange("0,1");
			qLastReview.setOrdering("dateCreated desc");
			List<Review> lastReviews = (List<Review>)qLastReview.execute(user.getKey());
			Dish lastReviewedDish = null;
			
			if(lastReviews.size() > 0){
				Review lastReview = lastReviews.get(0);
				lastReviewedDish = pm.getObjectById(Dish.class, lastReview.getDish());
			}
			
			Query qLastDish = pm.newQuery(Dish.class);
			qLastDish.setFilter("creator == :creatorParam");
			qLastDish.setRange("0,1");
			qLastDish.setOrdering("dateCreated desc");
			List<Dish> lastDishes = (List<Dish>)qLastDish.execute(user.getKey());
...
<%			if(lastReviewedDish != null){
				Restaurant rest = pm.getObjectById(Restaurant.class,lastReviewedDish.getRestaurant());
%>
			<div class="user_brief">
				<h3>Most recent rating:</h3>
				<div class="user_photo">
<%				if(lastReviewedDish.getPhotos() != null && lastReviewedDish.getPhotos().size() > 0){ %>
					<img class="grey_icon" src="/getPhoto?id=<%=lastReviewedDish.getPhotos().get(0).getId()%>"></img>
<%				}else{ %>
					<img class="grey_icon" src="style/no_user_img.jpg"></img>
<%				} %>	
				</div>
				<div class="user_info">
					<a href="#" class="dish_name">
					<%=lastReviewedDish.getName()%>
					</a>
					<p><%=rest.getName()%></p>
				</div>
			</div>
...
%>
			<div class="user_brief">
				...
				<div class="user_photo">
<%				if(lastDishAdded.getPhotos() != null && lastDishAdded.getPhotos().size() > 0){ %>
					<img class="grey_icon" src="/getPhoto?id=<%=lastDishAdded.getPhotos().get(0).getId()%>&h=98&w=109"></img>
<%				}else{ %>
					<img class="grey_icon" src="style/no_user_img.jpg"></img>
<%				} %>	
				</div>
