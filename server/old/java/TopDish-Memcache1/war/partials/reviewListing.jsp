...
<%
	...
	try{
		...
		r = pm.getObjectById(Review.class, reviewID);
		creator = pm.getObjectById(TDUser.class, r.getCreator());
...
<div class="dish_listing dish_splitter">
	...
	<%		if(creator.getPhoto() != null){ 
				Photo creatorPhoto = PMF.get().getPersistenceManager().getObjectById(Photo.class, creator.getPhoto());	
	%>
				<a href="userProfile.jsp?userID=<%=creator.getKey().getId()%>">
				<img class="grey_icon" src="<%=ImagesServiceFactory.getImagesService().getServingUrl(creatorPhoto.getBlobKey(), 36, true)%>"></img>
				</a>
	<%		}else{ %>
