...
<%
	...
	List<Dish> results = TDQueryUtils.getNewestDishes(1);
	...
	if(!results.isEmpty()){
		...
		Restaurant r = PMF.get().getPersistenceManager().getObjectById(Restaurant.class, d.getRestaurant());
    ...
    <div class="rmenu_disp">
    	<div class="top_dish">
        	...
        	<%	if(d.getPhotos() != null && d.getPhotos().size() > 0){
        			Photo p = PMF.get().getPersistenceManager().getObjectById(Photo.class, d.getPhotos().get(0));
        			try{
        			String url = ImagesServiceFactory.getImagesService().getServingUrl(p.getBlobKey(), 98, true);%>
        			<img class="dish_image_gold" src="<%=url%>" />
        	<%		}catch(Exception e){
        				System.err.print("Photo " + p.getKey().getId() + " has a problem!");
