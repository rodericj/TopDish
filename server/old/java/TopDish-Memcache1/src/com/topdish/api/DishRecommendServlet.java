...
public class DishRecommendServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		final SortedMap<Double, Dish> recoDishes = TDRecoUtils.recommendDishes(
				user, new Point(lat, lng), maxResults, maxDistance);
		...
		if (recoDishes != null) {
			for (Entry<Double, Dish> recoEntry : recoDishes.entrySet()) {
				try {
					...
					JSONObject jsonObj = new JSONObject((new Gson()
							.toJson(dishLite)));
