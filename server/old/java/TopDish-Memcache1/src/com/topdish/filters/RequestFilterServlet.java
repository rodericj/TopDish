...
public class RequestFilterServlet implements Filter{
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
		...
		if(UserServiceFactory.getUserService().isUserLoggedIn() && !UserServiceFactory.getUserService().isUserAdmin()) // restriction not needed if role is admin
		{
			try{
				...
				if(req.getRequestURI().contains("editDish.jsp") || req.getRequestURI().contains("updateDish")){
					...
					if(dishId!=null){System.out.println("dishid not null");
						if(!TDQueryUtils.isAccessible(Long.valueOf(dishId), new Dish(), pm, tdUser)){
					...
					if(restId!=null){
						if(!TDQueryUtils.isAccessible(Long.valueOf(restId), new Restaurant(), pm, tdUser)){
					...
					if(tagID!=null){
						if(!TDQueryUtils.isAccessible(Long.valueOf(tagID), new Restaurant(), pm, tdUser)){
