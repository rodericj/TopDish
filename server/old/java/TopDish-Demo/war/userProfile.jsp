...
<%
	...
	try{
		user = TDUserService.getUser(pm);		//will return valid user or throw exception
		if(user.getKey().getId() == userID || userID == 0){
...
<%
	...
	Query qReviews = pm.newQuery("select key from " + Review.class.getName());
	...
	Query qDishes = pm.newQuery("select key from " + Dish.class.getName());
