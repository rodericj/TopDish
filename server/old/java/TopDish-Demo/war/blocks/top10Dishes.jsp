...
<%
	...
	if(dishes != null && restaurants != null){
		Collections.sort(dishes, new DishPosReviewsComparator());
		if(dishes.size() > 10){			//if list is larger than 10 items
			dishes = dishes.subList(0,9); 	//get top 10 results and discard the rest
		}
		
		Collections.sort(restaurants, new RestaurantPosReviewsComparator());
...
<%					if(null != dishPhoto){
						String url = "";
						try{
							url = ImagesServiceFactory.getImagesService().getServingUrl(dishPhoto.getBlobKey(), 36, true);
	...
	<%				if(restPhoto != null){ 
						String url = "";
						try{
							url = ImagesServiceFactory.getImagesService().getServingUrl(restPhoto.getBlobKey(), 36, true);
