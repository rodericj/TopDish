...
<%					if(dishPhoto != null){ 
						try{
							String url = ImagesServiceFactory.getImagesService().getServingUrl(dishPhoto.getBlobKey(), 98, true);%>
...
<%          if (dishPhoto != null) { 
                try {
                    String dishPhotoUrl = ImagesServiceFactory.getImagesService().getServingUrl(dishPhoto.getBlobKey(), 98, true); %>
                   <img class="dish_thumbnail_image" src="<%=dishPhotoUrl%>" alt="<% out.print(d.getName()); %>"></img>
<%              } catch (Exception e) {
                    System.err.print("Photo " + dishPhoto.getKey().getId() + " has a problem!"); %>
...
<%          } %>
            <div class="info_window_text"> 
                <a href="dishDetail.jsp?dishID=<% out.print(d.getKey().getId()); %>"><% out.print(StringEscapeUtils.escapeHtml(d.getName())); %></a>
                <% out.print(" at "); %> 
                <a href="restaurantDetail.jsp?restID=<% out.print(r.getKey().getId()); %>"><% out.print(StringEscapeUtils.escapeHtml(r.getName())); %></a>
                <p><% out.print("Distance: " + TDMathUtils.formattedGeoPtDistanceMiles(userLoc, d.getLocation()) + " mi"); %></p>
            </div> 
        </div>
