...
public class TDRecoUtils {
	 ...
	 * @param tagKeys List of tag Keys
	 ...
	 * @param tagFreq Modifies input so that freqs are normalized
	...
	/**
	 * Filter tags that are not important given some tags (some tags can/will be repeated)
	 * @param tagKeys List of tags to process
	 * @return Returns tags that are considered important according to freq cutoff. 
	 */
	 ...
	 * @param profTags  Array of strings representing user profile.
	 * @param dishTags Array of strings representing dish
	 * @return similarity score between 0 and 1
	 */
	@SuppressWarnings("unused")
	private static double jaccardSim(List<String> profTags, List<String> dishTags) {
		Set<String> profTagSet = new HashSet<String>();
		Set<String> dishTagSet = new HashSet<String>();
		Set<String> unionSet = new HashSet<String>();    // Need this as tags can be repeated in input
	 ...
	 * @param profileTags Array of strings representing user profile
	 * @param dishTags Array of strings representing dish
	 * @return similarity score between 0 and 1
	 */
	 ...
	 * @param userKey Key of user of interest
	...
	private static List<Key> getTagsFromUser(TDUser user) {
		...
		List<Key> reviewKeys = TDQueryUtils.getReviewKeysByUser(user.getKey());
		
		for (Key reviewKey : reviewKeys) {
			try{
				Review review = pm.getObjectById(Review.class, reviewKey);
				Key dishKey = review.getDish();
				Dish dish = pm.getObjectById(Dish.class, dishKey);
	 ...
	 * 
	 * @param user User for whom we are recommending 
	 ...
	 * 
	 * @return SortedMap of predicted rating <Double> and Dish. Sorted by prediction.
	 */
	public static SortedMap<Double, Dish> recommendDishes(
		...
		try {
			List<Dish> recoDishes = (List<Dish>) GeocellManager.proximityFetch(location,
					maxResults, maxDistance, Dish.class, baseQuery, PMF.get()
							.getPersistenceManager());
			
			if (profTagList != null) {
				SortedMap<Double, Dish> dishPreds = new TreeMap<Double, Dish>(Collections.reverseOrder());
...
//					double posReviews = d.getNumPosReviews();
					...
					double sim = cosineSim(profTagList, dishTagKeys) * d.getPosReviewPercentage();
					dishPreds.put(sim, d);
	 ...
	 * @param maxDistance maximum distance to search in meters
	 * 
	 * @return SortedMap of predicted rating <Double> and restaurant. Sorted by prediction.
	 */
	public static SortedMap<Double, Restaurant> recommendRestaurants(
		...
		if (null != profTagList) {
			...
			// Profile is empty TODO tell user to rate some restaurants to get recos
			// Show message: More the ratings - better the recommendations
			//baseQuery = new GeocellQuery("", "", new ArrayList<Object>());
			return null;
		}
		...
		try {
			List<Restaurant> recoRestaurants = (List<Restaurant>) GeocellManager.proximityFetch(location,
					maxResults, maxDistance, Restaurant.class, baseQuery, PMF.get()
							.getPersistenceManager());
			
			if (profTagList != null) {
				SortedMap<Double, Restaurant> restPreds = new TreeMap<Double, Restaurant>(Collections.reverseOrder());
				...
				for (Restaurant rest : recoRestaurants) {
					...
					for (Key dishKey : rest.getDishes()) {
						Dish d = PMF.get().getPersistenceManager().getObjectById(Dish.class, dishKey);
...
//					double posReviews = d.getNumPosReviews();
					...
					if (restTagKeys.size() != 0) {
						double sim = cosineSim(profTagList, restTagKeys); // TBD need to impl * rest.getPosReviewPercentage();
