			...
			try{				
				if(req.getRequestURI().contains("editDish.jsp") || req.getRequestURI().contains("updateDish")){
					...
					if(dishId!=null){System.out.println("dishid not null");
						if(!TDQueryUtils.isAccessible(Long.valueOf(dishId), new Dish())){
					...
					if(restId!=null){
						if(!TDQueryUtils.isAccessible(Long.valueOf(restId), new Restaurant())){
					...
					if(tagID!=null){
						if(!TDQueryUtils.isAccessible(Long.valueOf(tagID), new Restaurant())){
